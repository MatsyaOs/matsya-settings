    ...
    : QApplication(argc, argv)
{
    ...
    setOrganizationName("matsyaos");

    QCommandLineParser parser;
    parser.setApplicationDescription(QStringLiteral("matsya Settings"));
    ...
    if (!QDBusConnection::sessionBus().registerService("com.matsya.SettingsUI")) {
        QDBusInterface iface("com.matsya.SettingsUI", "/SettingsUI", "com.matsya.SettingsUI", QDBusConnection::sessionBus());
        if (iface.isValid())
    ...
    // QML
    const char *uri = "Matsya.Settings";
    ...
    // Translations
    QLocale locale;
    QString qmFilePath = QString("%1/%2.qm").arg("/usr/share/matsya-settings/translations/").arg(locale.name());
